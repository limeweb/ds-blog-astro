---
title: "Airtable Reading Tracker: The Back-end"
...
import image1 from "@assets/posts/reading-app-airtable-backend-2.jpg";
import image2 from "@assets/posts/reading-app-fetch.png";
import image3 from "@assets/posts/reading-app-airtable-backend-3.jpg";
...
As is evident from this blog, I’m a big fan of Airtable, and so as I set out to build myself a custom reading tracker, it was naturally my first choice for a hybrid back-end/administrative interface to use for [a custom reading tracker](/posts/notes/imagining-the-ideal-reading-tracker-app).
...
Well, it turns out “too good to be true” is pretty accurate, because Airtable knows how powerful their platform is, and apparently they don’t believe this power should be wielded by mere individuals. (_Incipit_ rant—feel free to skip ahead.) As I was working on this project and writing this series of posts, Airtable [rolled out a new pricing structure](https://support.airtable.com/docs/changes-to-airtable-plans) that effectively quashes hobby use-cases like this.
...
<Figure
    src={image1}
    alt="A visualization of the Airtable base schema relationships, showing each table and its associated fields and how they are connected to other tables and fields."
    caption="The interconnections between all of the tables in the base."
/>
...
<CodeBlock title="Airtable Scripting Extension">
...
<Figure
    src={image2}
    alt="Airtable scripting interface showing the code from this post on the left side and the results of running the code on the right."
    caption="The Airtable scripting interface and sample test run of the fetch code
        above."
/>
...
<Figure
    src={image3}
    alt="Book record screen in Airtable showing the results of the fetch call applied to the record's fields."
    caption="Mapping the returned fetch results to the record’s fields."
/>

## To the Front-End

In the [next post](/posts/tutorials/airtable-reading-tracker-frontend), I’ll look at how I used the [Deno Fresh framework](https://fresh.deno.dev/) to build a server-side-rendered front-end to visually display all the reading data.
